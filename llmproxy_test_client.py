...
def test_models(base_url: str, **_) -> Optional[list[str]]:
    ...
    try:
        ...
        # Advertised form (GET /v1/models): "llmproxy/<name>" with any interior
        # "/" encoded as "__" (e.g. "llmproxy/deep__free"). The canonical
        # "llmproxy__<name>" form is also accepted as input.
        SYNTHETIC_IDS = {
            ...
            # Canonical form (interior "/" kept): accepted as input
        ...
        for _level in ("exploratory", "standard", "deep"):
            ...
            for _suffix in ("free", "local"):
                # Advertised combo form: "llmproxy/<level>__<suffix>".
...
def test_free_model(base_url: str, **_) -> None:
    ...
    # 'believed_free' detection: any candidate whose ID does not literally contain
    # 'free' must have been pulled in via the optional config['believed_free']
    # list, since the default rule matches only on the substring 'free'.
    believed_free_candidates = [c for c in candidates if "free" not in c.lower()]
    if believed_free_candidates:
        results.ok(
            "config['believed_free'] contributes candidate(s)",
            f"{len(believed_free_candidates)}: {', '.join(believed_free_candidates[:4])}"
            + (" ..." if len(believed_free_candidates) > 4 else ""),
        )
    else:
        results.skip(
            "config['believed_free'] contributes candidate(s)",
